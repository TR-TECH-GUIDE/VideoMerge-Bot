...
@SLBot.on_message(filters.private & filters.command("start"))
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                ...
                [InlineKeyboardButton("Support Group", url="https://t.me/trtechguide"),
                 InlineKeyboardButton("Bots Channel", url="https://t.me/SLBotsOfficial")],
...
@SLBot.on_message(filters.private & (filters.video | filters.document) & ~filters.edited)
...
@SLBot.on_message(filters.private & filters.photo & ~filters.edited)
...
@SLBot.on_message(filters.private & filters.command("settings"))
...
@SLBot.on_message(filters.private & filters.command("broadcast") & filters.reply & filters.user(Config.BOT_OWNER) & ~filters.edited)
async def _broadcast(_, m: Message):
    await broadcast_handler(m)


@SLBot.on_message(filters.private & filters.command("status") & filters.user(Config.BOT_OWNER))
...
@SLBot.on_message(filters.private & filters.command("check") & filters.user(Config.BOT_OWNER))
...
async def callback_handlers(bot: Client, cb: CallbackQuery):
    if "mergeNow" in cb.data:
        ...
        if Config.UPDATES_CHANNEL:
            try:
                ...
                if user.status == "kicked":
                    await cb.message.edit(
                        text="Sorry Sir, You are Banned to use me. Contact my [Support Group](https://t.me/tr-tech-guide).",
                        parse_mode="markdown",
                        disable_web_page_preview=True
                    )
                    return
        ...
        await cb.message.edit(
            text=Config.START_TEXT,
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Developer", url="https://t.me/TharukRenuja"), InlineKeyboardButton("Support Group", url="https://t.me/tr-tech-guide")], [InlineKeyboardButton("Bots Channel", url="https://t.me/SLBotsOfficial")]]),
            disable_web_page_preview=True
        )
        ...
        if (QueueDB.get(cb.from_user.id, None) is None) or (QueueDB.get(cb.from_user.id) == []):
            await cb.answer("Sorry Unkil, Your Queue is Empty!", show_alert=True)
            return
        merged_vid_path = f"{Config.DOWN_PATH}/{str(cb.from_user.id)}/[@TharukRenuja]_Merged.{FormtDB.get(cb.from_user.id).lower()}"
